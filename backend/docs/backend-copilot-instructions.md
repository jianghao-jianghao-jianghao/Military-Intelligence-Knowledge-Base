...
*   **Dependencies**: Use `Annotated` for dependency injection.
    *   *Correct*: `def read_items(user: Annotated[User, Depends(get_current_user)]):`
    *   *Incorrect*: `def read_items(user: User = Depends(get_current_user)):`
...
*   **Syntax**: Use strict 2.0 style.
    *   *Correct*: `result = await session.execute(select(User).where(User.id == user_id))`
    *   *Incorrect*: `user = session.query(User).filter_by(id=user_id).first()`
